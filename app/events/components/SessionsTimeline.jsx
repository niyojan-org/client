...
export default function SessionsTimeline({ event }) {
  ...
  return (
    ...
    >
      {/* ---- Header ---- */}
      <Card className="border-t-4 border-t-primary text-center backdrop-blur-md  shadow-xl hover:shadow-2xl transition-all duration-300 p-0 pt-1 pb-3 sm:p-4 gap-2">
        <CardHeader>
          <CardTitle className="text-2xl sm:text-3xl md:text-4xl font-bold text-primary">
            Sessions Timeline
          </CardTitle>
          <CardDescription className="text-sm sm:text-base md:text-lg max-w-2xl mx-auto text-muted-foreground ">
        ...
        <div className="relative px-4 pb-6 sm:px-6">
          ...
          <div
            className="hidden sm:block absolute left-8 top-6 bottom-6 w-px bg-primary/50"
            aria-hidden
          />
          ...
          <div className="space-y-8 sm:space-y-5">
            {sessions.map((session, idx) => (
              ...
              >
                ...
                {/* card */}
                <Card className="rounded-xl hover:shadow-lg transition-shadow duration-300 focus-within:ring-2 focus-within:ring-primary/40 p-2">
                  <CardContent className="sm:px-4 py-0 w-full px-0">
                    {/* header row */}
                    ...
                    {/* description */}
                    {session.description ? (
                      <CardDescription className="text-sm sm:text-base text-muted-foreground leading-relaxed mb-2 sm:mb-4 text-start">
                        {session.description}
                      </CardDescription>
                    ) : (
                      <CardDescription className="text-sm italic text-muted-foreground mb-2 sm:mb-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-1">
                      <div className="flex items-center gap-2 text-primary/90 text-sm sm:text-base">
                        <IconClock className="w-4 h-4 sm:w-5 sm:h-5 shrink-0" aria-hidden />
