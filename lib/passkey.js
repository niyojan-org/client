 ...
 */
export async function loginWithPasskey(userId, useConditionalUI = false, abortController = null) {
  if (!isPasskeySupported()) {
    throw new Error('Passkeys are not supported in this browser');
  }
  if (!userId) {
    throw new Error('User ID is required for passkey login');
  }
  try {
    // Step 1: Get login options from server
    // Server returns challenge, rpId, timeout, etc.
    const optionsResponse = await api.post('/auth/passkeys/authenticate/options', { userId });
    ...
    const verifyResponse = await api.post('/auth/passkeys/authenticate/verify', { userId, assertion: credentialJSON });
