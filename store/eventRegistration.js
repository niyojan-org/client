...
import {
  GetSuccessData,
  prepareGroupRegistrationPayload,
...
const useEventRegistrationStore = create((set, get) => ({
  ...
  fetchRegistrationForm: async (slug) => {
    try {
      ...
      const res = await api.get(`/registrations/${slug}/form`);
      const sortedTickets = sortTickets(res.data.event.tickets || []);
      set({ registrationForm: res.data.fields.fields });
  ...
  selectTicket: (id) => {
    if (!get().tickets) return;
    let ticket = null;
    if (id) {
      const idLower = id.toLowerCase();
      ticket = get().tickets.find((t) => t.id?.toLowerCase() === idLower || t.type?.toLowerCase() === idLower) || null;
    }
    set({ ticket });
  },
  register: async (formData) => {
    ...
    const { isValid, errors, firstErrorField } = validateFormData(formData, get().registrationForm);
    ...
    try {
      ...
      set({ successData: GetSuccessData(response.data.data) });
  ...
  registerGroup: async (groupData) => {
    try {
      ...
      const payload = prepareGroupRegistrationPayload(
        groupData,
        get().ticket._id,
        get().registrationForm,
        get().couponCode,
